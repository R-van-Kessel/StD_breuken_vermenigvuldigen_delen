...
function generateFractionMulDivQuestion() {
  ...
  // Genereer twee breuken
  let den1 = floor(random(2, 13));
  ...
  if (operation === '*') {
    ...
    // Delen = vermenigvuldig met omgekeerde
    answerNum = num1 * den2;
    answerDen = den1 * num2;
  }
  ...
  let questionText = num1 + '/' + den1 + ' ' + opSymbol + ' ' + num2 + '/' + den2 + ' = ?';
    ...
    if (block.text.includes('/')) {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (!isNaN(part) && !part.includes('/') && part !== '') {
                ...
                stroke(0);
                strokeWeight(2);
                line(startX - 7, baseY, startX + 7, baseY);
                noStroke();
