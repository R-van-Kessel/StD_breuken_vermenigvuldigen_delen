/* =====================================
   CSS STYLING (in DOM geïnjecteerd)
===================================== */
...
const TITLE_SPACE = 60;      // ↑ Meer ruimte boven titel
const BUTTON_HEIGHT = 70;   // Hoogte van knoppenrij
...
const SUBTITLE_Y = 385;

// DRAAK ACHTERGROND INSTELLINGEN - HIER KUN JE AANPASSEN!
const DRAGON_SCALE_X = 0.8;    // ← Horizontale schaal: 1.0=normaal, 1.5=breder, 0.5=smaller
const DRAGON_SCALE_Y = 0.6;    // ← Verticale schaal: 1.0=normaal, 1.5=hoger, 0.5=korter
...
function showInfo() {
    ...
    popup.innerHTML = `
        ...
        <p style="color: #0E0E0E; line-height: 1.2;">
            ...
            </ol><br><br><strong>Startpagina:</strong><br>Klik op "Summon the Dragon" om terug te gaan naar de algemene startpagina.
        </p>
...
//spatiebalk gebruik tijdens game
function keyPressed() {
...
function generateCompoundFractionQuestion() {
  ...
  // Genereer eerste gemengd getal: heel getal + breuk
  let whole1 = floor(random(0, 4));      // 0 tot 3 (kleiner voor vermenigvuldigen)
  let den1 = floor(random(2, 7));        // noemer 2-6 (kleiner voor eenvoudigere berekeningen)
  let num1 = floor(random(1, den1));     // teller < noemer (1 tot den1-1)
  
  // Genereer tweede gemengd getal
  let whole2 = floor(random(0, 4));      // 0 tot 3
  let den2 = floor(random(2, 7));        // noemer 2-6
  let num2 = floor(random(1, den2));     // teller < noemer (1 tot den2-1)
  ...
  // Bereken antwoord: a/b × c/d of a/b : c/d
  let answerNumerator;
  let answerDenominator;
  
  if (operation === '×') {
    // Vermenigvuldigen: (a/b) × (c/d) = (a×c)/(b×d)
    answerNumerator = improperNum1 * improperNum2;
    answerDenominator = den1 * den2;
  } else {
    // Delen: (a/b) : (c/d) = (a/b) × (d/c) = (a×d)/(b×c)
    answerNumerator = improperNum1 * den2;
    answerDenominator = den1 * improperNum2;
  }
...
function setup() {
    ...
    // Verander achtergrondkleur buiten canvas
    document.body.style.backgroundColor = '#0e1621';
    document.body.style.margin = '0';
    document.body.style.padding = '0';
    
    let canvasParent = cnv.elt.parentElement;
    if (canvasParent) {
    ...
    if (main) {
        main.style.backgroundColor = '#0e1621';
    }
}  // ← Sluithaakje staat nu op de juiste plek!
...
// Onderstreep bij hoveren
if (isHoveringTitle) {
    cursor(HAND);
    // Teken onderstreping
    let underlineY = TITLE_Y + TITLE_SIZE + 2;
    stroke(TITLE_COLOR[0], TITLE_COLOR[1], TITLE_COLOR[2]);
    strokeWeight(2);
    line(titleX - titleWidth/2, underlineY, titleX + titleWidth/2, underlineY);
    noStroke();
} else {
    cursor(ARROW);
}
...
function drawBlock(block) {
    ...
    if (block.text.includes('/')) {
        ...
        for (let part of parts) {
            ...
            else if (part === '+' || part === '-') estimatedWidth += 12;    // operator optellen/aftrekken
            else if (part === '×' || part === ':') estimatedWidth += 12;    // operator vermenigvuldigen/delen
