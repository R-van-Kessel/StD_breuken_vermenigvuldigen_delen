...
function createNavigation() {
  ...
  nav.innerHTML = `
    ...
    <a href="https://r-van-kessel.github.io/Summon_the_Dragon/overig.html">🎓 Overig</a>
  `;
...
function generateFractionMulDivQuestion() {
  
  // Kies willekeurig vermenigvuldigen (*) of delen (/)
  let operation = random() < 0.5 ? '*' : '/';

  // Genereer eerste breuk — 50% kans op gemengd getal
  let den1 = floor(random(2, 9));
  let num1 = floor(random(1, den1));
  let whole1 = random() < 0.5 ? floor(random(1, 3)) : 0;

  // Converteer eerste naar onechte breuk voor berekening
  let improperNum1 = whole1 * den1 + num1;

  // Genereer tweede breuk (altijd gewone breuk)
  let den2 = floor(random(2, 13));
  let num2 = floor(random(1, den2));

  // Bereken antwoord
  let answerNum, answerDen;
  if (operation === '*') {
    answerNum = improperNum1 * num2;
    answerDen = den1 * den2;
  } else {
    answerNum = improperNum1 * den2;
    answerDen = den1 * num2;
  }

  // Vereenvoudig
  function gcd(a, b) { return b === 0 ? a : gcd(b, a % b); }
  let d = gcd(answerNum, answerDen);
  answerNum = answerNum / d;
  answerDen = answerDen / d;

  // Maak vraag en antwoord strings
  let opSymbol = operation === '*' ? 'x' : ':';
  let firstPart = whole1 > 0 ? whole1 + ' ' + num1 + '/' + den1 : num1 + '/' + den1;
  let questionText = firstPart + ' ' + opSymbol + ' ' + num2 + '/' + den2 + ' = ?';

  let answerText;
  if (answerDen === 1) {
    answerText = answerNum.toString();       // Heel getal
  } else {
    answerText = answerNum + '/' + answerDen; // Breuk
  }
...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        let q = generateFractionMulDivQuestion();
        questions.push({ text: q.question, answer: q.answer });
        answers.push({ text: q.answer, answer: q.answer });
    }
...
for (let btn of canvasButtons) {
    // Update hover voor alle buttons (ook op mobiel voor smooth fade-out)
    if (!draggingBlock) {
        btn.checkHover(mouseX, mouseY);
    } else {
        btn.hoverProgress = lerp(btn.hoverProgress, 0, 0.15);
    }
    btn.draw();
}
...
function drawBlock(block) {
  push();
   ...
   // Teken blokje (gebruik relatieve positie als getransformeerd)
    ...
    // Bepaal base positie voor tekenen
    let baseX = (block.isQuestion && block.hoverProgress > 0) ? CELL_SIZE / 2 : block.x + CELL_SIZE / 2;
    let baseY = (block.isQuestion && block.hoverProgress > 0) ? CELL_SIZE / 2 : block.y + CELL_SIZE / 2;
    
    // Check of de tekst breuken bevat
    if (block.text.includes('/')) {
        // Split de tekst in delen
        let parts = block.text.split(' ');
        
        // Schat de totale breedte
        let estimatedWidth = 0;
        for (let part of parts) {
            if (!isNaN(part) && !part.includes('/')) estimatedWidth += 15;  // heel getal
            else if (part.includes('/')) estimatedWidth += 20;              // breuk
            else if (part === '+' || part === '-') estimatedWidth += 15;    // operator
            else if (part === '=' || part === '?') estimatedWidth += 15;    // equals/vraagteken
        }
        
        // Start positie
        let startX = baseX - estimatedWidth / 2 + 5;
        
        for (let i = 0; i < parts.length; i++) {
            let part = parts[i];
            
            if (!isNaN(part) && !part.includes('/') && part !== '') {
                // Heel getal van gemengd getal
                textSize(15);
                text(part, startX, baseY);
                startX += 15;
                
            } else if (part.includes('/')) {
                // BREUK met horizontale streep
                let [num, den] = part.split('/');
                
                textSize(15);
                fill(0);
                noStroke();
                text(num, startX, baseY - 10);  // Teller
                
                // Breukstreep
                stroke(0);
                strokeWeight(2);
                line(startX - 9, baseY, startX + 9, baseY);
                noStroke();
                
                fill(0);
                text(den, startX, baseY + 10);  // Noemer
